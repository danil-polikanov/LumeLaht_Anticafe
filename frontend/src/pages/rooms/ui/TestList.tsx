...
export const TestList = () => {
  ...
  const cities = Array.from(new Set(rooms.map((room) => room.address?.city).filter(Boolean)));
  const activities = Array.from(
    new Set(rooms.flatMap((room) => room.activity?.map((a) => a.name) || []).filter(Boolean)),
  );
  ...
  const filteredAndSortedRooms = useMemo(() => {
    let filtered = rooms.filter((room) => {
      ...
      const matchesCity = !selectedCity || room.address?.city === selectedCity;

      const matchesActivity =
        !selectedActivity || room.activity?.some((activity) => activity.name === selectedActivity);

      const matchesMinPrice = !minPrice || (room.pricePerHour || 0) >= parseFloat(minPrice);
      const matchesMaxPrice = !maxPrice || (room.pricePerHour || 0) <= parseFloat(maxPrice);

      return matchesSearch && matchesCity && matchesActivity && matchesMinPrice && matchesMaxPrice;
    });
    ...
    return filtered;
  }, [rooms, searchTerm, selectedCity, selectedActivity, minPrice, maxPrice, sortBy]);
        ...
        <div className="bg-white shadow-sm border-b">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex items-center justify-between h-16">
              ...
              <h1 className="text-xl font-semibold text-gray-900">Детали комнаты</h1>
              <div className="w-20" />
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
            ...
            <div className="relative h-96 bg-gray-200">
              <img
                ...
                alt={`${selectedRoom.name} - изображение ${currentImageIndex + 1}`}
            ...
            <div className="p-8">
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div className="lg:col-span-2">
                  <h1 className="text-3xl font-bold text-gray-900 mb-4">{selectedRoom.name}</h1>
                  ...
                  <div className="mb-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {selectedRoom.activity?.map((activity, index) => (
                        <span
                          key={index}
                    ...
                    <div>
                      ...
                      <div className="space-y-2 text-gray-600">
                        <div className="flex items-center">
                          ...
                          <span>{selectedRoom.address?.addressName}</span>
                        </div>
                        <div className="ml-6">
                          <div>
                            {selectedRoom.address?.city}, {selectedRoom.address?.region}
                          </div>
                          <div>
                            {selectedRoom.address?.postalCode}, {selectedRoom.address?.country}
                          </div>
                        </div>
                      </div>
                    </div>
                    ...
                    <div>
                      ...
                      <div className="text-gray-600">
                        <div className="flex items-center">
                          ...
                          <span>{selectedRoom.address?.phoneNumber}</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="bg-gray-50 rounded-xl p-6 sticky top-8">
                    ...
                    <div className="space-y-3 text-sm text-gray-600">
                      <div className="flex items-center justify-between">
                        ...
                        >
                          {selectedRoom.isActive ? 'Активна' : 'Неактивна'}
                        </span>
                      </div>
                      <div className="flex items-center justify-between">
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white shadow-sm border-b">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-16">
            <h1 className="text-2xl font-bold text-gray-900">Поиск комнат</h1>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <div className="flex-1">
            {/* Results Info */}
            <div className="flex items-center justify-between mb-6">
              <div className="text-gray-600">Найдено {filteredAndSortedRooms.length} комнат</div>
            </div>
                  ...
                  <div className="p-6">
                    <div className="flex items-start justify-between mb-3">
                      ...
                      <div className="text-right">
                        <div className="text-xl font-bold text-gray-900">{room.pricePerHour}€</div>
                    ...
                    <div className="flex items-center text-sm text-gray-600 mb-3">
                      ...
                      <span>
                        {room.address?.city}, {room.address?.addressName}
                      </span>
                    </div>

                    <div className="flex flex-wrap gap-2 mb-4">
                      {room.activity?.slice(0, 2).map((activity, index) => (
                        <span
                          key={index}
                      ...
                      {(room.activity?.length || 0) > 2 && (
                        <span className="px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs">
                          +{(room.activity?.length || 0) - 2} еще
                        </span>
                      )}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      >
                        {room.isActive ? 'Доступна' : 'Недоступна'}
                      </span>
