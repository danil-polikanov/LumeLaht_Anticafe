  ...
  if (state.filters.search) {
    filtered = filtered.filter(
      (room) =>
        ...
        room.description?.toLowerCase().includes(state.filters.search.toLowerCase()),
    );
  }

  if (state.filters.city) {
    filtered = filtered.filter((room) =>
      room.address?.city?.toLowerCase().includes(state.filters.city.toLowerCase()),
    );
  }

  if (state.filters.region) {
    filtered = filtered.filter((room) =>
      room.address?.region?.toLowerCase().includes(state.filters.region.toLowerCase()),
    );
  }

  if (state.filters.minPrice > 0) {
    filtered = filtered.filter((room) => (room.pricePerHour || 0) >= state.filters.minPrice);
  }

  if (state.filters.maxPrice < 10000) {
    filtered = filtered.filter((room) => (room.pricePerHour || 0) <= state.filters.maxPrice);
  }
