...
namespace LumeLaht_RoomApi.Tests.Controllers
{
    public class BookingControllerTests
    {
        ...
        public BookingControllerTests()
        {
            ...
            // Authenticated user via a fake ClaimsPrincipal — the controller
            // pulls UserId from NameIdentifier; we simulate that.
        ...
        public async Task GetMyBookings_Returns200_WithCurrentUsersBookingsOnly()
        {
            ...
            {
                new() { BookingId = Guid.NewGuid(), Status = "Confirmed" },
                new() { BookingId = Guid.NewGuid(), Status = "Cancelled" },
            };
            ...
            // Critical: controller must scope by the JWT user id, not by query param.
        ...
        public async Task GetRoomBookings_Returns200_WithListForGivenDate()
        {
            ...
            var bookings = new List<BookingResponse>
            {
                new() { BookingId = Guid.NewGuid(), RoomId = roomId, StartTime = date.AddHours(10) },
            };
        ...
        {
            // Service-thrown exceptions are caught by the global ExceptionHandlingMiddleware
            // and translated to HTTP status codes; controller itself just lets them bubble.
            var request = new CreateBookingRequest { RoomId = Guid.NewGuid(), StartTime = DateTime.UtcNow.AddHours(2) };
            _serviceMock.Setup(s => s.CreateBookingAsync(_userId, request, It.IsAny<CancellationToken>()))
